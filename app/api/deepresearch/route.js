...
export async function POST(request) {
  try {
    ...
    return Response.json(
      // { status: resp.status, output_text: resp.output_text, id: resp.id, resp: resp }
      { "sending": "deep research request to backend", "status": "success" }
    ); // Return the response with output_type
  } catch (error) {
